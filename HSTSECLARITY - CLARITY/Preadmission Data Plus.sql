...
SELECT DISTINCT
              ...
              ,sthx.UPDATE_USER_ID
			  ,sthx.UPDATE_TIME
			  --,sthx.UPDATED_ENC_STAT_C -- ZC_PAT_STATUS
			  --,zps2.NAME AS UPDATED_ENC_STAT_NAME
			  --,sthx.UPDATED_CONF_STAT_C -- ZC_CONF_STAT
			  --,zcs2.NAME AS UPDATED_CONF_STAT_NAME
			  ,emp.NAME 'UPDATE_USER_NAME'
			  ,sthx.UPDATE_EVENT_ID
			  ,hsp.CONTACT_DATE
			  ,hsp.HSP_ACCOUNT_ID
			  ,hsp.HOSP_ADMSN_TIME
			  ,hsp.EXP_ADMISSION_TIME
			  ,hsp.ADMIT_SOURCE_C
...
INNER JOIN CLARITY.dbo.PAT_ENC_HSP hsp				ON sthx.PAT_ENC_CSN_ID = hsp.PAT_ENC_CSN_ID
...
LEFT OUTER JOIN CLARITY.dbo.ZC_HOV_CONF_STATUS	zhcs		ON zhcs.HOV_CONF_STATUS_C = hsp.HOV_CONF_STATUS_C
LEFT OUTER JOIN CLARITY.dbo.ZC_CONF_STAT zcs						ON zcs.ADMIT_CONF_STAT_C = hsp.ADMIT_CONF_STAT_C
--LEFT OUTER JOIN CLARITY.dbo.ZC_CONF_STAT zcs2						ON zcs.ADMIT_CONF_STAT_C = sthx.UPDATED_CONF_STAT_C
LEFT OUTER JOIN CLARITY.dbo.ZC_ACCT_CLASS_HA zach			ON zach.ACCT_CLASS_HA_C = hsp.ADT_PAT_CLASS_C
...
--LEFT OUTER JOIN CLARITY.dbo.ZC_PAT_STATUS zps2					ON zps2.ADT_PATIENT_STAT_C = sthx.UPDATED_ENC_STAT_C
LEFT OUTER JOIN CLARITY.dbo.ZC_ADM_CATEGORY zac				ON zac.ADMIT_CATEGORY_C = hsp.ADMIT_CATEGORY_C
/*
...
--
--final filters to pull hospital encounters, drop canceled preadmissions and pre-admits that did not result in a hospital stay
                     WHERE  fpec.sk_Dim_Clrt_Enc_Typ=71	--sk:	71=Hospital Encounter (3)
                            AND (fpehc.ADMIT_CONF_STAT NOT IN ('Canceled', 'Pending') OR fpehc.ADMIT_CONF_STAT IS NULL)
                            AND (fpehc.ADT_PATIENT_STAT<>'Preadmission' OR fpehc.ADT_PATIENT_STAT IS NULL)	--for patient 2403822 admission 9/20/2013 why does record viewer show EPT 10115 = Preadmission and DW show ADT_PATIENT_STAT = Confirmed (not same item?)
                            --AND dcac.ADMIT_CATEGORY<>'Inpatient Pre-admit'
							AND (dcp.TEST = 0 OR dcp.test IS NULL) -- Exclude test Patients
*/
WHERE 1=1
 AND hsp.CONTACT_DATE>='7/1/2020'
 AND hsp.CONTACT_DATE<'3/1/2021'
 --AND sthx.UPDATED_ENC_STAT_C = '1' --PREADMISSION
 --AND  sthx.UPDATED_CONF_STAT_C = '1' --CONFIRMED PREADMISSION
 --AND ((sthx.UPDATED_ENC_STAT_C = '1' --PREADMISSION
 --           AND  sthx.UPDATED_CONF_STAT_C = '1') --CONFIRMED PREADMISSION
	--	   OR (hsp.ED_EPISODE_ID IS NOT NULL)
	--	  )
 --AND (
 --          (
 --           ((zcs.NAME NOT IN ('Canceled', 'Pending') OR zcs.NAME IS NULL)
 --            AND (zcs2.NAME NOT IN ('Canceled', 'Pending') OR zcs2.NAME IS NULL)
	--	    )
	--	   AND hsp.ADT_PATIENT_STAT_C <> 6 -- Hospital Outpatient Visit
	--	   )
	--	   OR (hsp.ED_EPISODE_ID IS NOT NULL)
	--	  )
 AND (
...
--ORDER BY PAT_ENC_CSN_ID
 ...
 SELECT PAT_CLASS_NAME,
        ...
        UPDATE_TIME,
  --      UPDATED_ENC_STAT_C,
	 --   UPDATED_ENC_STAT_NAME,
  --      UPDATED_CONF_STAT_C,
		--UPDATED_CONF_STAT_NAME,
        UPDATE_EVENT_ID,
        HSP_ACCOUNT_ID,
        HOSP_ADMSN_TIME,
        EXP_ADMISSION_TIME,
...
--Authi/Cert Status
	...
	--	,enc.REFERRAL_ID				-- remove syg2d 2021.04.14
...
SELECT  DISTINCT
	--pnd.PEND_ID
	 hx.CONTACT_DATE
--	,hx.PAT_ID
--	,ISNULL(hx.HOSP_ADMSN_TIME,hx.EXP_ADMISSION_TIME) 'HOSP_ADMSN_TIME'
--	,hx.COVERAGE_ID
--	,zcrdc.NAME		'Pend_Record_Type'
	,idx.IDENTITY_ID	'MRN'
	 --idx.IDENTITY_ID	'MRN'
...
--Authi/Cert Status
	...
	,certsts.NAME						'AUTHCERT_STATUS'
	,har.ASSOC_AUTHCERT_ID				-- added syg2d 2021.04.14
	--	,hx.REFERRAL_ID				-- remove syg2d 2021.04.14
...
LEFT OUTER JOIN CLARITY.dbo.ZC_PA_UNDO_RSN undorsn			ON cncl.UPDATE_REASON_EPT_C = undorsn.PREADM_UNDO_RSN_C
 ...
 --AND hsp.CONTACT_DATE>='7/1/2020'
 AND hsp.CONTACT_DATE>='2/22/2021'
 AND hsp.CONTACT_DATE<'3/1/2021'
...
ORDER BY hsp.CONTACT_DATE
                   ,hsp.PAT_ENC_CSN_ID
*/
INNER JOIN CLARITY.dbo.IDENTITY_ID idx						ON hx.PAT_ID = idx.PAT_ID
...
INNER JOIN  (SELECT --TOP 50 
				...
				LEFT OUTER JOIN CLARITY.dbo.CLARITY_EMP emp
				ON emp.USER_ID = rhx.REG_HX_USER_ID
			) reg											ON hx.PAT_ENC_CSN_ID =	reg.CSN AND reg.RNo='1' -- first user to touch registration
			...
			INNER JOIN  (SELECT --TOP 50 
				...
				LEFT OUTER JOIN CLARITY.dbo.CLARITY_EMP emp
				ON emp.USER_ID = rhx.REG_HX_USER_ID
			) regend											ON hx.PAT_ENC_CSN_ID =	regend.CSN AND regend.RNo='1' -- first user to touch registration

LEFT OUTER JOIN CLARITY.dbo.COVERAGE cvg					ON hx.COVERAGE_ID = cvg.COVERAGE_ID
...
LEFT OUTER JOIN (
					...
					)cvgvrx									ON hx.PAT_ENC_CSN_ID = cvgvrx.ENC_CSN
...
LEFT OUTER JOIN (
					...
					)harvrx									ON hx.HSP_ACCOUNT_ID = harvrx.VERIF_RECORD_IDNT 
																AND hx.HOSP_ADMSN_TIME>=harvrx.LAST_STAT_CHNG_DTTM
																AND CAST(hx.HOSP_ADMSN_TIME AS DATE)<=CAST(harvrx.NEXT_REVIEW_DATE AS DATE)
...
LEFT OUTER JOIN (
				...
				AND hx.UPDATED_CONF_STAT_C ='1' --CONFIRMED
				AND hx.UPDATED_ENC_STAT_C IN ('2','6') --ADMISSIONS /HOV
				)admhx											ON admhx.PAT_ENC_CSN_ID = hx.PAT_ENC_CSN_ID AND admhx.RNo='1'




--PATIENT TYPE
LEFT OUTER JOIN CLARITY.dbo.PATIENT_TYPE pttyp				ON hx.PAT_ID	=	pttyp.PAT_ID
LEFT OUTER JOIN CLARITY.dbo.ZC_PATIENT_TYPE zcpttyp			ON pttyp.PATIENT_TYPE_C = zcpttyp.PATIENT_TYPE_C


--AUTH/CERT
LEFT OUTER JOIN CLARITY.dbo.HSP_ACCOUNT			har			ON	har.HSP_ACCOUNT_ID		= hx.HSP_ACCOUNT_ID	-- added syg2d 2021.04.14
LEFT OUTER JOIN CLARITY.dbo.REFERRAL_CVG_AUTH		rfl			ON	har.COVERAGE_ID			= rfl.AUTH_CERT_CVG_ID	-- added syg2d 2021.04.14
																and har.ASSOC_AUTHCERT_ID	= rfl.REFERRAL_ID		-- added syg2d 2021.04.14
--LEFT OUTER JOIN CLARITY.dbo.REFERRAL_CVG_AUTH rfl			ON hx.COVERAGE_ID = rfl.AUTH_CERT_CVG_ID
--																and hx.REFERRAL_ID = RFL.REFERRAL_ID

--MSPQ
LEFT OUTER JOIN CLARITY.dbo.PAT_ENC_MSP mspq				ON mspq.PAT_ENC_CSN_ID = hx.PAT_ENC_CSN_ID
...
LEFT OUTER JOIN (
				SELECT  doc.*, emp.NAME
					...
					LEFT OUTER JOIN CLARITY.dbo.CLARITY_EMP emp ON emp.USER_ID = doc.RECV_BY_USER_ID
					WHERE doc.DOC_INFO_TYPE_C IN ('204100014') --Insurance Waiver Form
					)dcswv									ON hx.PAT_ENC_CSN_ID = dcswv.DOC_CSN
...
LEFT OUTER JOIN (
				SELECT  doc.*, emp.NAME
					...
					LEFT OUTER JOIN CLARITY.dbo.CLARITY_EMP emp ON emp.USER_ID = doc.RECV_BY_USER_ID
					WHERE doc.DOC_INFO_TYPE_C IN ('204100012') -- Important Message from Medicare
					)dcsmc									ON hx.PAT_ENC_CSN_ID = dcsmc.DOC_CSN
...
LEFT OUTER JOIN (
				SELECT  doc.*, emp.NAME
					...
					LEFT OUTER JOIN CLARITY.dbo.CLARITY_EMP emp ON emp.USER_ID = doc.RECV_BY_USER_ID
					WHERE doc.DOC_INFO_TYPE_C IN ('310100001')--Notice of Admission
					)dcsadm									ON hx.PAT_ENC_CSN_ID = dcsadm.DOC_CSN
...
LEFT OUTER JOIN (
					...
					WHERE 1=1
					AND rte1.RTE_PAYOR_ID IS NOT NULL
				)rte										ON rte.PAT_ID = hx.PAT_ID AND CAST(hx.HOSP_ADMSN_TIME AS DATE) =CAST(rte.RTE_SENT_TRUE_DTTM AS DATE)

																

--MYCHART STATUS
LEFT OUTER JOIN CLARITY.dbo.PATIENT_MYC myc					ON hx.PAT_ID = myc.PAT_ID


--SELF PAYMENT 
LEFT OUTER JOIN CLARITY.dbo.PAT_ENC_SELF_PMT slfpmt			ON hx.PAT_ENC_CSN_ID = slfpmt.PAT_ENC_CSN_ID
...
LEFT OUTER JOIN (
					...
					WHERE  1=1
					AND  hx.UPDATED_ENC_STAT_C ='3' --cancelled
					) cncl															ON hx.PAT_ENC_CSN_ID = cncl.PAT_ENC_CSN_ID
...
--HB
--INNER JOIN CLARITY.dbo.HSP_TRANSACTIONS AS TX 
--ON hx.HSP_ACCOUNT_ID = TX.HSP_ACCOUNT_ID 
--AND tx.TX_TYPE_HA_C <> 1


LEFT OUTER JOIN CLARITY.dbo.ZC_PAT_SERVICE svc				ON hx.HOSP_SERV_C = svc.HOSP_SERV_C
LEFT OUTER JOIN CLARITY.dbo.ZC_ADM_SOURCE admsrc			ON hx.ADMIT_SOURCE_C = admsrc.ADMIT_SOURCE_C 
LEFT OUTER JOIN CLARITY.dbo.ZC_HOSP_ADMSN_TYPE admtyp		ON hx.HOSP_ADMSN_TYPE_C = admtyp.HOSP_ADMSN_TYPE_C
LEFT OUTER JOIN CLARITY.dbo.CLARITY_DEP dep					ON hx.DEPARTMENT_ID = dep.DEPARTMENT_ID
LEFT OUTER JOIN CLARITY.dbo.ZC_ACCOMMODATION acm			ON hx.ACCOMMODATION_C = acm.ACCOMMODATION_C
LEFT OUTER JOIN CLARITY.dbo.ZC_PRE_CERT_STATUS certsts		ON rfl.PRE_CERT_STATUS_C = certsts.PRE_CERT_STATUS_C
LEFT OUTER JOIN CLARITY.dbo.ZC_MSP_COMPLETION zcmsp			ON mspq.MSP_STATUS_C = zcmsp.MSP_COMPLETION_C
LEFT OUTER JOIN CLARITY.dbo.ZC_DOC_STAT zcwvst				ON dcswv.DOC_STAT_C = zcwvst.DOC_STAT_C
LEFT OUTER JOIN CLARITY.dbo.ZC_DOC_STAT zcmcst				ON dcsmc.DOC_STAT_C = zcmcst.DOC_STAT_C
...
LEFT OUTER JOIN CLARITY.dbo.ZC_RTE_RESP_STATUS zcrtest		ON rte.RTE_RESP_STATUS_C = zcrtest.RTE_RESP_STATUS_C
LEFT OUTER JOIN CLARITY.dbo.ZC_MYCHART_STATUS zcmyc			ON myc.MYCHART_STATUS_C = zcmyc.MYCHART_STATUS_C
LEFT OUTER JOIN CLARITY.dbo.ZC_CANCEL_REASON cnlrsn			ON hx.CANCEL_REASON_C = cnlrsn.CANCEL_REASON_C
LEFT OUTER JOIN CLARITY.dbo.CLARITY_EPM epm					ON cvg.PAYOR_ID = epm.PAYOR_ID
LEFT OUTER JOIN CLARITY.dbo.CLARITY_EPP epp					ON cvg.PLAN_ID = epp.BENEFIT_PLAN_ID
LEFT OUTER JOIN CLARITY.dbo.ZC_PA_UNDO_RSN undorsn			ON cncl.UPDATE_REASON_EPT_C = undorsn.PREADM_UNDO_RSN_C
