...
SELECT DISTINCT
	...
	--,tx.BILLED_AMOUNT
	--,tx.COINSURANCE_AMOUNT
	--,tx.COPAY_AMOUNT
	--,tx.DEDUCTIBLE_AMOUNT
	--,tx.TX_AMOUNT
	--,tx.NON_COVERED_AMT
...
SELECT [EVENT_ID]
 ...
 --                   , ADM_EVENT_ID
  ...
  ON CSNs.PAT_ENC_CSN_ID = Rhx.REG_HX_OPEN_PAT_CSN
	 ...
	 --   LEFT OUTER JOIN CLARITY.dbo.ZC_REG_HX_EVENT_TYPE zrhet
		--ON zrhet.REG_HX_EVENT_C = Rhx.REG_HX_EVENT_C
...
ORDER BY CSN
  ...
  SELECT hx.PAT_ENC_CSN_ID,
         hx.UPDATE_USER_ID,
         hx.UPDATE_TIME--,
   --      hx.UPDATED_ENC_STAT_C,
		 --zps.NAME AS UPDATED_ENC_STAT_NAME,
   --      hx.UPDATED_CONF_STAT_C,
		 --zcs.NAME AS UPDATED_CONF_STAT_NAME,
		 --hx.RNo
