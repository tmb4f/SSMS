...
SELECT @StartDate AS StartDate, 
  ...
  WHERE ddend.CALENDAR_DT = CAST(CAST(@EndDate AS DATE)  AS DATETIME)
  )
...
availbase as (
   select 
		  financial_division = CLARITY_SER.RPT_GRP_SIX,
		  division_name = div.FIN_DIV_NM,
		  financial_subdivision = CLARITY_SER.RPT_GRP_EIGHT,
		  subdivision_name = subdiv.FIN_SUBDIV_NM,
          avail.department_id, 
	...
	LEFT OUTER JOIN CLARITY.dbo.FIN_SUBDIV subdiv 	ON subdiv.FIN_SUBDIV_ID = CAST(CLARITY_SER.RPT_GRP_EIGHT AS NUMERIC(18,0))
    LEFT OUTER JOIN CLARITY.dbo. FIN_DIV div ON div.FIN_DIV_ID = subdiv.FIN_DIV_ID

   WHERE 1=1
   AND slot_begin_time >=  dates.StartDate  
   AND  SLOT_BEGIN_TIME < dates.EndDateExcl
   AND div.FIN_DIV_ID = '48000' -- Family Medicine
   --AND CLARITY_SER.PROV_ID = '28801' -- ALLEN, CLAUDIA
   AND CLARITY_SER.PROV_ID = '478008' -- GRAVATTE IV, LEROY T

)

SELECT * FROM availbase
--where  department_id in (@department)
ORDER BY availbase.department_name, availbase.provider, availbase.SLOT_DATE, availbase.SLOT_BEGIN_TIME
