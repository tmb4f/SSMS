...
--AS
	...
	DECLARE @EndDate DATETIME;
			...
			--SET  @StartDate= '5/1/2021 00:00:00';
			SET  @StartDate= '7/1/2022 00:00:00';
			--SET  @EndDate	='5/6/2021 23:59:59';
			--SET  @EndDate	='2/28/2022 23:59:59';
			SET  @EndDate	='10/23/2022 23:59:59';
...
GROUP BY hsp.PAT_ID,hsp.VISIT_PROV_ID, hsp.APPT_STATUS_C
)
/**********************************************/
...
--AND enc.VISIT_PROV_ID IN ('109152','72660')
)
/************************************************************/
...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	,DATEDIFF (YEAR, pt.BIRTH_DATE, lst_cte.Adm_Dt) AS Age
	,edg.ICD9_CODE
	,rd.Acc_ID
	,lst_cte.PRIMARY_LOC_ID
	,loc.RPT_GRP_NINE
	,ZEAF9.NAME AS RPT_GRP_NINE_NAME
...
FROM CLARITY.dbo.CLARITY_SER ser
			...
			LEFT OUTER JOIN CLARITY.dbo.CLARITY_LOC loc		ON loc.LOC_ID = lst_cte.PRIMARY_LOC_ID
...
--	         END
--	       ELSE LTRIM(gacc.ACCOUNT_NAME)
--	     END AS VARCHAR(30)) LIKE '%donor%'
			

--ORDER BY	CAST(ser.PROV_ID AS VARCHAR(30))
		--	,CAST(idx.IDENTITY_ID AS VARCHAR(12))   DESC
ORDER BY	CAST(pt.PAT_ID AS VARCHAR(20)), lst_cte.Adm_Dt
/****************************************************************************************/
