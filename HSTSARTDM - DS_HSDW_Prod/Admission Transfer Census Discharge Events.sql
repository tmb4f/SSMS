...
inpatient admission, transfer, census, and discharge count over a specific date range.
...
INFO:	
	...
	OUTPUTS:
			...
			Detail list of reportable events for an encounter
--------------------------------------------------------------------------------------------
MODS:
...
if OBJECT_ID('tempdb..#EventsSum ') is not null
Drop table #EventsSum
...
SELECT distinct hclrt.PAT_ENC_CSN_ID
  ...
  AND (
	((adte.day_date < @strBegindt) AND ((ddte.date_key = 19000101) OR (ddte.day_date >= @strBegindt))) -- Admits prior to reporting period
	OR
    ((adte.day_date >= @strBegindt) AND (adte.day_date <= @strEnddt)) -- Admits during reporting period
  )
...
--ORDER BY sk_Adm_Dte, sk_Dsch_Dte
...
INTO #EventsSum
FROM #RptgEvents
WHERE Event_Clrt_DEPt_Nme LIKE 'TCIR%'
AND ADT_Evnt_Nme = 'Census'
--ORDER BY sk_Fact_Pt_Enc_Clrt
--       --, ADT_In_Dtm
--	   , Seq_Num_in_Enc
...
SELECT
	Event_DEPARTMENT_ID AS EPIC_DEPARTMENT_ID,
	Event_Clrt_DEPt_Nme AS EPIC_DEPARTMENT_NAME,
	CAST(Event_Date AS DATE) AS CENSUS_DATE,
	day_of_week AS DOW,
