...
select 
  ...
  where ddend.CALENDAR_DT = @EndDate
  )
...
availbase AS (
   ...
   AND avail.PROV_ID = '29528'

)

SELECT * FROM availbase
--where  department_id in (@department)
ORDER BY availbase.DEPARTMENT_ID, availbase.SLOT_DATE, availbase.SLOT_BEGIN_TIME
