...
--SET @InsertIntoString = N';WITH cte_pods_servLine (pod_Service_Line) AS (SELECT Param FROM ETL.fn_ParmParse(@PodServiceLine, '','')) INSERT INTO #RecallDepartment SELECT DISTINCT mdm.epic_department_id AS DepartmentIdFROM CLARITY.[dbo].[CL_SSA] ssa INNER JOIN CLARITY.[dbo].[CL_SSA_PROV_DEPT] pd ON ssa.ACTION_ID = pd.ACTION_ID LEFT OUTER JOIN CLARITY.dbo.CLARITY_DEP dep ON dep.DEPARTMENT_ID = pd.DEP_ID LEFT OUTER JOIN Rptg.vwRef_MDM_Location_Master_EpicSvc mdm ON pd.DEP_ID = mdm.epic_department_id WHERE 1=1 AND mdm.epic_department_id IS NOT NULL AND (ssa.RECALL_DT >= @RecallStartDate AND ssa.RECALL_DT <= @RecallEndDate) AND COALESCE(' + @DepartmentGrouperColumn + ',''' + @DepartmentGrouperNoValue + ''') IN (SELECT pod_Service_Line FROM cte_pods_servLine) ORDER BY DepartmentId';
--SET @InsertIntoString = N'DECLARE @RecallDepartment TABLE (DepartmentId NUMERIC(18,0), PatId VARCHAR(18), ApptStatus INTEGER, ContactDate DATETIME, NextSeq INTEGER, PrevSeq INTEGER);WITH cte_pods_servLine (pod_Service_Line) AS (SELECT Param FROM ETL.fn_ParmParse(@PodServiceLine, '','')) INSERT INTO @RecallDepartment SELECT DISTINCT mdm.epic_department_id AS DepartmentId, ssa.PAT_ID AS PatId, appt.APPT_STATUS_C AS ApptStatus, appt.CONTACT_DATE AS ContactDate, appt.Next_Seq AS NextSeq, appt.Prev_Seq AS PrevSeq FROM CLARITY.[dbo].[CL_SSA] ssa INNER JOIN CLARITY.[dbo].[CL_SSA_PROV_DEPT] pd ON ssa.ACTION_ID = pd.ACTION_ID LEFT OUTER JOIN CLARITY.dbo.CLARITY_DEP dep ON dep.DEPARTMENT_ID = pd.DEP_ID LEFT OUTER JOIN Rptg.vwRef_MDM_Location_Master_EpicSvc mdm ON pd.DEP_ID = mdm.epic_department_id LEFT OUTER JOIN (SELECT [DEPARTMENT_ID], [PAT_ID], [APPT_STATUS_C], [CONTACT_DATE], ROW_NUMBER() OVER (PARTITION BY PAT_ID, DEPARTMENT_ID, APPT_STATUS_C ORDER BY CONTACT_DATE ASC) AS Next_Seq, ROW_NUMBER() OVER (PARTITION BY PAT_ID, DEPARTMENT_ID, APPT_STATUS_C ORDER BY CONTACT_DATE DESC) AS Prev_Seq FROM [CLARITY].[dbo].[V_SCHED_APPT] WHERE ((APPT_STATUS_C = 2 AND CONTACT_DATE <= CAST(GETDATE() AS DATE)) OR (APPT_STATUS_C = 1 AND CONTACT_DATE >= CAST(GETDATE() AS DATE)))) appt ON appt.DEPARTMENT_ID = dep.DEPARTMENT_ID AND appt.PAT_ID = ssa.PAT_ID WHERE 1=1 AND mdm.epic_department_id IS NOT NULL AND (appt.Next_Seq IS NULL OR appt.Next_Seq = 1) AND (appt.Prev_Seq IS NULL OR appt.Prev_Seq = 1) AND (ssa.RECALL_DT >= @RecallStartDate AND ssa.RECALL_DT <= @RecallEndDate) AND COALESCE(' + @DepartmentGrouperColumn + ',''' + @DepartmentGrouperNoValue + ''') IN (SELECT pod_Service_Line FROM cte_pods_servLine) ORDER BY DepartmentId, PatId; SELECT * FROM @RecallDepartment ORDER BY DepartmentId, PatId, ContactDate;';
--SET @InsertIntoString = N';WITH cte_pods_servLine (pod_Service_Line) AS (SELECT Param FROM ETL.fn_ParmParse(@PodServiceLine, '','')) INSERT INTO #RecallDepartment SELECT DISTINCT mdm.epic_department_id AS DepartmentId, ssa.PAT_ID AS PatId, appt.APPT_STATUS_C AS ApptStatus, appt.CONTACT_DATE AS ContactDate, appt.Next_Seq AS NextSeq, appt.Prev_Seq AS PrevSeq FROM CLARITY.[dbo].[CL_SSA] ssa INNER JOIN CLARITY.[dbo].[CL_SSA_PROV_DEPT] pd ON ssa.ACTION_ID = pd.ACTION_ID LEFT OUTER JOIN CLARITY.dbo.CLARITY_DEP dep ON dep.DEPARTMENT_ID = pd.DEP_ID LEFT OUTER JOIN Rptg.vwRef_MDM_Location_Master_EpicSvc mdm ON pd.DEP_ID = mdm.epic_department_id LEFT OUTER JOIN (SELECT [DEPARTMENT_ID], [PAT_ID], [APPT_STATUS_C], [CONTACT_DATE], ROW_NUMBER() OVER (PARTITION BY PAT_ID, DEPARTMENT_ID, APPT_STATUS_C ORDER BY CONTACT_DATE ASC) AS Next_Seq, ROW_NUMBER() OVER (PARTITION BY PAT_ID, DEPARTMENT_ID, APPT_STATUS_C ORDER BY CONTACT_DATE DESC) AS Prev_Seq FROM [CLARITY].[dbo].[V_SCHED_APPT] WHERE ((APPT_STATUS_C = 2 AND CONTACT_DATE <= CAST(GETDATE() AS DATE)) OR (APPT_STATUS_C = 1 AND CONTACT_DATE >= CAST(GETDATE() AS DATE)))) appt ON appt.DEPARTMENT_ID = dep.DEPARTMENT_ID AND appt.PAT_ID = ssa.PAT_ID WHERE 1=1 AND mdm.epic_department_id IS NOT NULL AND (appt.Next_Seq IS NULL OR appt.Next_Seq = 1) AND (appt.Prev_Seq IS NULL OR appt.Prev_Seq = 1) AND (ssa.RECALL_DT >= @RecallStartDate AND ssa.RECALL_DT <= @RecallEndDate) AND COALESCE(' + @DepartmentGrouperColumn + ',''' + @DepartmentGrouperNoValue + ''') IN (SELECT pod_Service_Line FROM cte_pods_servLine) ORDER BY DepartmentId, PatId';
...
--SELECT DISTINCT
--	DepartmentId
--  , cldep.DEPARTMENT_NAME
--FROM #RecallDepartment dep
----FROM @RecallDepartment
--LEFT OUTER JOIN CLARITY..CLARITY_DEP cldep
--ON cldep.DEPARTMENT_ID = dep.DepartmentId
...
ORDER BY CAST(evnts.PAT_MRN_ID AS INTEGER)';
SET @ParmDefinition = N'@RecallStartDate SMALLDATETIME, @RecallEndDate SMALLDATETIME, @PodServiceLine VARCHAR(MAX), @DepartmentId VARCHAR(MAX)';
